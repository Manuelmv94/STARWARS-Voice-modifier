...
public class Plot 
{
	...
	public Plot( JFrame ventana)
	{
		...
		try
		{
			SwingUtilities.invokeAndWait( new Runnable() {
				...
				{
					frame = ventana;
					BufferedImage img = new BufferedImage( 512, 370, BufferedImage.TYPE_4BYTE_ABGR );
					Graphics2D g = (Graphics2D)img.getGraphics();
					g.setColor( Color.black );
	...
	{			
		...
		{						
			...
			{
				image = new BufferedImage( (int)(samples.length / samplesPerPixel), 350, BufferedImage.TYPE_4BYTE_ABGR );
				Graphics2D g = image.createGraphics();
				g.setColor( Color.black );
				g.fillRect( 0, 0, image.getWidth(), image.getHeight() ); 
				g.dispose();
